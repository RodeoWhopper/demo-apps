...
A small local operator UI for the demo apps in this repository. One page lists every app with its live
...
```bash
cd 00-control-panel && npm install && npm start
```
...
docker run -d --name demo-control-panel -p 8000:8000 \
  -e HOST=0.0.0.0 -e PANEL_TOKEN=change-me -e DEMO_APPS_ROOT="$(cd .. && pwd)" \
...
| `DEMO_APPS_ROOT` | no | parent folder | Where the `NN-*` folders live |
...
- `lib/catalog.js` — scans `../NN-*/deploy.json`, merges `catalog.json` extras (setup scripts)
