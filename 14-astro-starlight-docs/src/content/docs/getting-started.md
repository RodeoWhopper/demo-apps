...
- A project with a `Dockerfile`, a `docker-compose.yml`, or one of the [detected framework presets](/configuration/#framework-presets).
...
See [Installation](/installation/) for Homebrew, npm and Docker options.
...
```text
✔ Detected: Dockerfile (multi-stage, node:22-alpine)
