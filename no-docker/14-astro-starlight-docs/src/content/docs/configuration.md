...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `containerfile` | path | auto-detected | Path to the Containerfile relative to `context`. |
| `context` | path | `.` | Build context uploaded to the builder. Respects `.containerignore`. |
| `target` | string | — | Multi-stage build target. |
| `args` | table | `{}` | Build arguments. Values may use `secret://` references. |
| `preset` | string | auto-detected | Force a [framework preset](#framework-presets) instead of a Containerfile. |
...
When no Containerfile is present, Kestrel checks for these in order and generates a build for you:

| Preset | Detected by | Notes |
|--------|-------------|-------|
| `compose` | `compose.yaml` / `compose.yml` | Every service is deployed; the first `ports` entry becomes the route. |
