...
#   - with --snapshot, exports the schema to ./snapshot/schema.yaml via `docker compose exec directus`
...
if [ "${1:-}" = "--snapshot" ]; then
  ...
  docker compose exec -T directus npx directus schema snapshot --yes /directus/snapshot/schema.yaml
