...
# Usage:
#   scripts/create-admin.sh            # local (runs npx strapi inside ./cms; the server may be running or stopped)
#   scripts/create-admin.sh --docker   # inside the running compose service "cms"
...
set +e
if [ "${1:-}" = "--docker" ]; then
  OUT="$(docker compose exec -T cms npx strapi admin:create-user --email="$EMAIL" --password="$PASSWORD" --firstname="$FIRST" --lastname="$LAST" 2>&1)"
else
