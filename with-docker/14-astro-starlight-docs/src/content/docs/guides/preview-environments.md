...
Inside the environment the app reaches them at `db:5432` and `cache:6379`. Pass connection strings through `[build].args` or runtime env in your Dockerfile as usual.

## Compose projects

If your repository has a `docker-compose.yml`, Kestrel uses the `compose` preset: every service is deployed, `depends_on` ordering is respected, and the first service with a `ports` entry becomes the default route. `build:` sections are built remotely; `image:` services are pulled.
